  ...
  async function getResourceServerConfig(token) {
...

    {
      ...
      if (token.resourceServer) {
        if (token.resourceServer.jwt && token.resourceServer.jwt.sign) {
    ...
    if (token.resourceServer && token.resourceServer.jwt && token.resourceServer.jwt.encrypt) {
      ...
      if (!(key instanceof crypto.KeyObject) && /^(A|P|dir$)/.test(alg)) {
        key = crypto.createSecretKey(key);
      }
  ...
  return {
    ...
    async getValueAndPayload() {
      ...
      if (config.sign && config.encrypt) {
        ...
        return { value: encrypted, payload };
      }

      if (config.sign) {
        return { value: signed, payload };
      }
